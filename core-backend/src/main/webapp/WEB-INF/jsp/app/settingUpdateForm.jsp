...
{	
	...
	$("#btn_cont_save").click(function(){
		...
		var option = {
				url : '${ctx}/adm/settingUpdateSave.do',
				dataType : 'json',				
							...
							<div class="panel-body">
								...
								<input type="hidden" id="CON_ID" name="CON_ID" value="${detail.CON_ID }">						
									...
									<div class="form-group">
	                                    ...
	                                    <div class="col-lg-11">
	                                    	<input type="text" class="form-control"  id="CON_SUBTITLE" name="CON_SUBTITLE" value="${detail.CON_TITLE}" title="부제목" />                         	
	                                    </div>
	                                </div>
	                                </c:if>
	                                <div class="form-group">
	                                    ...
	                                    <div class="col-lg-11">
	                                    	<c:if test="${fn:length(detail.CON_THUMNAIL) > 0}">
												<img src="${detail.CON_THUMNAIL}" height="200" width="200"/>
											</c:if>
	                                ...
	                                <div class="form-group">
	                                    ...
	                                    <div class="col-lg-11">
	                                    	 <textarea name="CON_BODY" id="CON_BODY" style="width: 100%">${detail.CON_BODY}</textarea>                         	
	                                    </div>
	                                </div>
	                                <!-- 홍보마당 , 스토리 -->
									 ...
									 <div class="form-group">
										...
										<div class="col-lg-4">	       
											<select name="CON_CATEGORY">
												<option value="0" <c:if test="${detail.CON_CATEGORY == 0}">selected</c:if>>싱글남</option>
												<option value="1" <c:if test="${detail.CON_CATEGORY == 1}">selected</c:if>>싱글녀</option>
												<option value="2" <c:if test="${detail.CON_CATEGORY == 2}">selected</c:if>>전문직</option>
												<option value="3" <c:if test="${detail.CON_CATEGORY == 3}">selected</c:if>>직장인</option>
												<option value="4" <c:if test="${detail.CON_CATEGORY == 4}">selected</c:if>>고교생</option>
												<option value="5" <c:if test="${detail.CON_CATEGORY == 5}">selected</c:if>>대학생</option>
												<option value="6" <c:if test="${detail.CON_CATEGORY == 6}">selected</c:if>>공무원</option>
												<option value="7" <c:if test="${detail.CON_CATEGORY == 7}">selected</c:if>>CEO(사장님)</option>
											</select>
										</div>
	                                </div>	
									</c:if>     
